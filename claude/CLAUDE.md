...
Use the `/amnesia` skill for all memory operations. **At session start, invoke
`/amnesia` before anything else.** The skill contains all the instructions.
...
design direction, your NEXT tool call must be `amnesia save` — before writing
code, before reading files, before anything else.** This includes:
